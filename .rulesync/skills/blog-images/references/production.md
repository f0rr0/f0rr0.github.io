...
Keep Markdown production notes in `research/blog-images/<slug>.md`, outside `src/content/blog`. The current broad dynamic imports enumerate the article tree; a co-located `image-notes.md` is picked up as an unsupported module by a fresh Turbopack compile. Keep only runtime article modules and supported assets there.
